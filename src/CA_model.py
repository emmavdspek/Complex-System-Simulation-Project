...
def update_Scanlon2007_local_only(
    ...
    M=25,
    dmin=1,
    BC="periodic",  # fp = current fraction of trees
):
    ...
    for i, j in cells_to_update:
        ...
        for dx in range(-M, M + 1):
            for dy in range(-M, M + 1):
                dist = np.sqrt(
                    (dx) ** 2 + (dy) ** 2
                )  # distand between cell and neighbor, gives closer neighbors more influence
                ...
                if BC == "periodic":  # the grid never "ends" like a donut-shaped grid
                    ...
                    if 0 < dist <= M:
                        ...
                        numerator += (dmin / dist) ** k * current_grid[
                            (i + dx) % size,
                            (j + dy)
                            % size,  # this % makes it wrap around the grid (comes back on the other side)
                        ]

        # define parameters necessary for the update rule
        rho = numerator / cum_distr  # rho_t in the paper (neighbourhood tree density)
        frac_occ = np.sum(current_grid) / (
            size**2
        )  # fraction of vegetation, f_t in paper
        random_nr = np.random.random()  # number to use for updating
    ...
    for i, j in cells_to_update:
        ...
        for dx in range(-M, M + 1):
            for dy in range(-M, M + 1):
                ...
                if (
                    BC == "aperiodic"
                ):  # meaning you are on the edge and no neighbors next to you
                    # if the neighbor is within bounds and within range, add contributions
                    if (
                        (0 <= (i + dx) < size)
                        and (0 <= (j + dy) < size)
...
def update_Scanlon_exp(
    ...
    for i, j in cells_to_update:
        ...
        rho = numerator / (
            cum_distr + eps
        )  # rho_t in the paper, the epsilon avoid the division by 0 when using only local or global rule
        frac_occ = np.sum(current_grid) / (size**2)  # fraction of vegetation
        ...
        if current_grid[i, j] == 0:
            ...
            if (
                random_nr < prob_flip
            ):  # of the weighted probability is higher, then change into a tree
                grid[i, j] = 1
        else:
            prob_flip = (
                phi * (1 - rho) + omega * global_term
            ) * 2  # since both probability can be up to 1, they add up to 2
            prob_flip = min(max(prob_flip, 0.0), 1.0)
            if random_nr < prob_flip:
                grid[i, j] = 0
...
def evolve_CA(
    ...
    for n in range(N_steps):
        ...
        cells_to_update = np.column_stack(
            [np.random.randint(0, size, N_update), np.random.randint(0, size, N_update)]
        )
