...
def evolve_CA(
    ...
    for n in range(N_steps):
        ...
        # update the grid one step, depending on the model we use (we pass more or less arguments)
        if update_rule.__name__ == "update_basic":
            update_args = [cells_to_update]

        elif update_rule.__name__ in [
             "update_Scanlon2007",
             "update_Scanlon_exp",
             "update_Scanlon2007_global_only", #maybe we can delete 
             "update_Scanlon2007_deterministic"
             ]: 
                update_args = [cells_to_update, true_frac, k, M, phi]
        
        grid = update_rule(grid, *update_args) #give the current grid and selected cells, and return a new grid 
